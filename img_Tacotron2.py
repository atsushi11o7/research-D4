...
class ConvNorm(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1,
        ...
        if padding is None:
            assert(kernel_size % 2 == 1)  # kernel_sizeが奇数はない
...
class Attention(nn.Module):
    ...
    def get_alignment_energies(self, query, processed_memory, attention_weights_cat):
        ...
        #print("pro_query", processed_query.shape)
        processed_attention_weights = self.location_layer(attention_weights_cat)
        #print("pro_att", processed_attention_weights.shape)
        #print("pro_memory", processed_memory.shape)
        energies = self.v(torch.tanh(
            processed_query + processed_attention_weights + processed_memory))
        #print("energies", energies.shape)
        energies = energies.squeeze(-1)
        #print("energies_2", energies)
        return energies
    ...
    def forward(self, attention_hidden_state, memory, processed_memory,
        ...
        alignment = alignment.masked_fill_(mask, -float("inf"))

        attention_weights = F.softmax(alignment, dim=1)
        #print("att_wei", attention_weights.shape)
        attention_context = torch.bmm(attention_weights.unsqueeze(1), memory)
        #print("att_context", attention_context.shape)
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.attention_rnn = nn.LSTMCell(
            prenet_hidden_dim + encoder_hidden_dim + attention_dim, hidden_dim)
        ...
        # 画像処理層
        self.image_layer = LinearNorm(encoder_hidden_dim, attention_dim, bias=False, w_init_gain='tanh')  # query2つ目
    ...
    def decode(self,
        ...
        cell_input = torch.cat((cell_input, pro_image_frame), -1)  # ついでに映像フレーム(1, 128)もくっつける
        ...
        return (decoder_output, attention_hidden,
    ...
    def forward(self, memory,
        ...
        pro_2_image_frame = self.image_layer(pro_image_frame)

        # (B, T, emb_dim) -> (T, B, emb_dim)
        pro_2_image_frame = pro_2_image_frame.transpose(0, 1)  # 0次元目と1次元目の入れ替え

        if is_inference:
            #max_decoder_steps = self.inference_max_decoder_steps  # 推論時は出力上限1000に設定
            max_decoder_steps = pro_2_image_frame.size(0)  # 推論時の出力の長さは、映像フレーム数
        else:
        ...
        mask = self.get_mask_from_lengths(memory_lengths).to(memory.device)
...
class Tacotron2(nn.Module):
    ...
    def forward(self, inputs, input_lengths, pro_image_frame, decoder_targets):
        """
        PARAMS
        -------
        inputs: 入力となるテキスト情報（音素列）
        input_lengths: 入力の長さ
        pro_image_frame: (B, T, 512)
        ...
        mel_outputs, alignments = self.decoder(encoder_outputs, input_lengths, pro_image_frame, decoder_targets)
    ...
    def inference(self, inputs, input_lengths, pro_image_frame):
        """
        """

        mel_outputs, mel_outputs_postnet, alignments = self.forward(inputs, input_lengths, pro_image_frame, None)
